...
class TestValidateBreakerSpec:
    ...
    def test_invalid_af_value(self):
        ...
        assert exc_info.value.details["field"] == "breaker.frame_af"
    ...
    def test_invalid_32af_3p(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_breaker_spec(breaker)
        # expected 필드에 허용 극수 정보가 있음
        assert "[2]" in exc_info.value.details["expected"]
    ...
    def test_invalid_400af_2p(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_breaker_spec(breaker)
        # expected 필드에 허용 극수 정보가 있음
        assert "[3, 4]" in exc_info.value.details["expected"]
    ...
    def test_invalid_general_pole_combination(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_breaker_spec(breaker)
        # expected 필드에 허용 극수 정보가 있음
        assert "허용 극수" in exc_info.value.details["expected"]
...
class TestNormalizeDimensionUom:
    ...
    def test_invalid_string(self):
        ...
        # expected 필드에 형식 정보가 있음
        assert "100mm" in exc_info.value.details["expected"]
...
class TestValidateDimensionRange:
    ...
    def test_below_min(self):
        ...
        # expected 필드에 범위 정보가 있음
        assert "100.0~1000.0mm" in exc_info.value.details["expected"]
...
class TestValidateEnclosureInput:
    ...
    def test_invalid_main_breaker(self):
        ...
        # expected 필드에 32AF 허용 극수 정보가 있음
        assert "[2]" in exc_info.value.details["expected"]
    ...
    def test_invalid_branch_breaker_first(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_enclosure_input(main, branches)
        # field에 branch_breakers[0]이 포함됨
        assert "branch_breakers[0]" in exc_info.value.details["field"]
    ...
    def test_invalid_branch_breaker_second(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            validate_enclosure_input(main, branches)
        # field에 branch_breakers[1]이 포함됨
        assert "branch_breakers[1]" in exc_info.value.details["field"]
