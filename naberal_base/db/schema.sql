...
-- ============================================================================
-- VIEWS (UNRESTRICTED - No RLS)
-- ============================================================================

-- v_quote_summary: Quote summary view (unrestricted access)
CREATE OR REPLACE VIEW estimator.v_quote_summary AS
SELECT
    q.id,
    q.status,
    q.customer->>'name' as customer_name,
    q.customer->>'company' as customer_company,
...
COMMENT ON VIEW estimator.v_quote_summary IS 'Quote summary with aggregated counts (unrestricted access)';

-- v_phase_balance: Phase balance analysis view (unrestricted access)
CREATE OR REPLACE VIEW estimator.v_phase_balance AS
SELECT
    p.id as panel_id,
    p.quote_id,
    p.name as panel_name,
    SUM(CASE WHEN b.phase_assignment = 'R' THEN (b.capacity::INTEGER * b.qty) ELSE 0 END) as phase_r_load,
    SUM(CASE WHEN b.phase_assignment = 'S' THEN (b.capacity::INTEGER * b.qty) ELSE 0 END) as phase_s_load,
    SUM(CASE WHEN b.phase_assignment = 'T' THEN (b.capacity::INTEGER * b.qty) ELSE 0 END) as phase_t_load,
    COUNT(*) as breaker_count
FROM estimator.panels p
LEFT JOIN estimator.breakers b ON b.panel_id = p.id
GROUP BY p.id, p.quote_id, p.name;

COMMENT ON VIEW estimator.v_phase_balance IS 'Phase load balance analysis per panel (unrestricted access)';
...
-- Note: Views (v_quote_summary, v_phase_balance) are intentionally unrestricted for monitoring/analytics
