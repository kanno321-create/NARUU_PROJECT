...
async def create_task(request: CreateTaskRequest):
    ...
    try:
        ...
        logger.error(f"할 일 생성 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"할 일 생성 중 오류 발생: {str(e)}"
        )
...
async def get_tasks(
    ...
    try:
        ...
        logger.error(f"할 일 목록 조회 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"목록 조회 중 오류 발생: {str(e)}"
        )
...
async def update_task(task_id: str, request: UpdateTaskRequest):
    ...
    try:
        ...
        logger.error(f"할 일 수정 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"수정 중 오류 발생: {str(e)}"
        )
...
async def suggest_priorities():
    ...
    try:
        ...
        logger.error(f"우선순위 제안 실패: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"제안 중 오류 발생: {str(e)}"
        )
