...
export function DepositSlipWindow() {
    ...
    const handleItemChange = (id: string, field: keyof DepositItem, value: string | number) => {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            await api.erp.payments.create({
                ...
                customer_id: items[0]?.accountCode || "deposit",
                amount: totalAmount,
                payment_method: "계좌이체",
                memo: items.map(i => `${i.bankName} ${i.accountName} ${i.note}`.trim()).filter(Boolean).join(", ") || undefined,
            });
            alert(`예금입금전표가 저장되었습니다.\n총 입금액: ${totalAmount.toLocaleString()}원`);
        } catch (err) {
            alert(`예금입금전표 저장 실패: ${err}`);
        } finally {
            setIsSaving(false);
        }
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex-1 overflow-auto p-4">
                <div className="space-y-4">
                    <div className="rounded-lg border border-green-200 bg-green-50 p-2 text-center text-sm text-green-700">예금입금전표 - 계좌 입금 처리용</div>
                    <div className="grid grid-cols-4 gap-4">
                        ...
                        <div className="col-span-3 flex items-end">
                            <div className="rounded-lg bg-green-100 px-6 py-2 text-center">
                                <div className="text-xs text-green-600">입금 합계</div>
                                <div className="text-xl font-bold text-green-700">{totalAmount.toLocaleString()} 원</div>
                            </div>
                        </div>
                    </div>
