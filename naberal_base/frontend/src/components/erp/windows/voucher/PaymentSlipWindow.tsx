...
export function PaymentSlipWindow() {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            await api.erp.payments.create({
                ...
                customer_id: supplierCode || supplierName,
                amount: totalAmount,
                payment_method: items[0]?.type || "현금",
                memo: items.map(i => i.note).filter(Boolean).join(", ") || undefined,
            });
            alert(`지급전표가 저장되었습니다.\n거래처: ${supplierName || supplierCode}\n총 지급액: ${totalAmount.toLocaleString()}원`);
        } catch (err) {
            alert(`지급전표 저장 실패: ${err}`);
        } finally {
            setIsSaving(false);
        }
    }, [isSaving, supplierCode, supplierName, totalAmount, slipDate, items]);
            ...
            <div className="flex-1 overflow-auto p-4">
                <div className="space-y-4">
                    <div className="grid grid-cols-4 gap-4">
                        ...
                        <div><label className="mb-1 block text-sm font-medium">거래처코드</label><input type="text" value={supplierCode} onChange={(e) => setSupplierCode(e.target.value)} className="w-full rounded border px-3 py-2 text-sm" placeholder="F2: 검색" /></div>
                        <div className="col-span-2"><label className="mb-1 block text-sm font-medium">거래처명</label><input type="text" value={supplierName} onChange={(e) => setSupplierName(e.target.value)} className="w-full rounded border px-3 py-2 text-sm" /></div>
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-surface">
                                <tr className="border-b">
                                    ...
                                    <th className="w-32 px-2 py-2 text-left">지급유형</th>
