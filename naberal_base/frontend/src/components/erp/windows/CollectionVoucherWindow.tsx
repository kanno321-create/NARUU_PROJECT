...
// ORIGINAL_EMPLOYEES, ORIGINAL_BANKS, ORIGINAL_CARDS replaced with API-loaded state
...
export function CollectionVoucherWindow() {
  ...
  useEffect(() => {
    ...
    api.erp.customers.list({ search: customerSearchQuery || undefined, limit: 50 }).then(res => {
      setApiCustomers(res.items.map((c: any) => ({
        ...
        salesCommission: c.sales_commission || 0,
      })));
    }).catch(() => setApiCustomers([]));
  }, [showCustomerModal, customerSearchQuery]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api.erp.customers.list({ search: customerSearch, limit: 10 }).then(res => {
  ...
  // 기본정보
  const [collectionDate, setCollectionDate] = useState("2025년 12월 05일");
  ...
  // 필터된 데이터 (API에서 이미 필터링됨)
  const filteredCustomers = apiCustomers;
  ...
  // 결제 타입별 합계
  const cashTotal = paymentItems.filter(i => i.type === "현금").reduce((sum, i) => sum + i.amount, 0);
  const bankTotal = paymentItems.filter(i => i.type === "은행").reduce((sum, i) => sum + i.amount, 0);
  const billTotal = paymentItems.filter(i => i.type === "어음").reduce((sum, i) => sum + i.amount, 0);
  const cardTotal = paymentItems.filter(i => i.type === "카드").reduce((sum, i) => sum + i.amount, 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`수금전표가 저장되었습니다.\n거래처: ${selectedCustomer.name}\n총 수금액: ${totalAmount.toLocaleString()}원`);
    } catch (err) {
      alert(`수금전표 저장에 실패했습니다.\n거래처: ${selectedCustomer.name}\n총 수금액: ${totalAmount.toLocaleString()}원\n(오프라인 저장)`);
    }
  };
  ...
  // 현재 탭에 해당하는 결제 항목만 필터
  const currentTabItems = paymentItems.filter(item => item.type === activePaymentTab);
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <label className="w-20 text-sm text-gray-600">수금한일자</label>
                <select
              ...
              <div className="relative flex items-center gap-2">
                <label className="text-sm text-gray-600">수금한거래처(O)</label>
                <input
                  type="text"
              ...
              {/* 수금담당(U) */}
              <div className="flex items-center gap-2">
                <label className="text-sm text-gray-600">수금담당(U)</label>
                <input
                  type="text"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 border-r border-gray-200 p-4">
          ...
          <div className="space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-600">수금시에누리:</span>
                <input
                  type="number"
                  value={discount || ""}
              ...
              <div className="h-4 w-40 rounded bg-blue-200" style={{ width: `${Math.min(100, (discount / (totalAmount || 1)) * 100)}%` }} />
            </div>
          </div>
        </div>
          ...
          {selectedCustomer ? (
            <div className="space-y-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">미수금액:</span>
                  <span className="font-medium text-red-600">{selectedCustomer.receivable.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-600">판매수수료:</span>
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      value={selectedCustomer.salesCommission}
                      readOnly
                      className="w-16 rounded border border-gray-300 bg-white px-2 py-1 text-right text-sm"
                    />
          ...
          <div className="w-[500px] rounded-lg bg-white shadow-xl">
            ...
            <div className="p-4">
                ...
                <table className="w-full text-sm">
                  <thead className="sticky top-0 bg-gray-100">
                    <tr>
                      ...
                      <th className="border-b border-gray-200 px-3 py-2 text-right font-medium text-gray-600">미수금액</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredCustomers.map((customer) => (
                      ...
                      >
                        <td className="border-b border-gray-100 px-3 py-2">{customer.code}</td>
                        <td className="border-b border-gray-100 px-3 py-2">{customer.name}</td>
                        <td className="border-b border-gray-100 px-3 py-2">{customer.representative}</td>
              ...
              <div className="mt-3 flex justify-between text-sm text-gray-500">
                <span>총 {filteredCustomers.length}개 검색됨</span>
              </div>
            </div>
