...
interface ProductCategoryMargin {
    id: string;
    categoryCode: string;
    categoryName: string;
...
export function ProductCategoryMarginWindow() {
    const [margins, setMargins] = useState<ProductCategoryMargin[]>([
        { id: "1", categoryCode: "CAT001", categoryName: "차단기", marginGroupCode: "MG001", marginGroupName: "일반", marginRate: 20, isActive: true },
        { id: "2", categoryCode: "CAT002", categoryName: "배선용차단기", marginGroupCode: "MG002", marginGroupName: "도매", marginRate: 10, isActive: true },
    ]);

    const [searchQuery, setSearchQuery] = useState("");
    const [selectedId, setSelectedId] = useState<string | null>(null);
    const [isEditing, setIsEditing] = useState(false);
    const [editForm, setEditForm] = useState<Partial<ProductCategoryMargin>>({});

    const filteredMargins = margins.filter((m) => m.categoryName.includes(searchQuery) || m.marginGroupName.includes(searchQuery));

    const handleAdd = () => {
        const newMargin: ProductCategoryMargin = { id: String(Date.now()), categoryCode: "", categoryName: "", marginGroupCode: "", marginGroupName: "", marginRate: 0, isActive: true };
        setMargins([...margins, newMargin]); setSelectedId(newMargin.id); setEditForm(newMargin); setIsEditing(true);
    };

    const handleEdit = (margin: ProductCategoryMargin) => { setSelectedId(margin.id); setEditForm(margin); setIsEditing(true); };
    const handleSave = () => { if (selectedId && editForm.categoryName) { setMargins(margins.map((m) => (m.id === selectedId ? { ...m, ...editForm } as ProductCategoryMargin : m))); setIsEditing(false); } };
            ...
            <div className="flex flex-1 overflow-hidden">
                <div className="w-1/2 overflow-auto border-r">
                    <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-surface"><tr className="border-b"><th className="px-3 py-2 text-left">품목분류</th><th className="px-3 py-2 text-left">마진그룹</th><th className="px-3 py-2 text-right">마진율</th><th className="px-3 py-2 text-center">사용</th><th className="px-3 py-2 text-center">관리</th></tr></thead>
                        <tbody>
                            {filteredMargins.map((margin) => (
                                <tr key={margin.id} className={`border-b cursor-pointer hover:bg-surface-secondary ${selectedId === margin.id ? "bg-brand/10" : ""}`} onClick={() => handleEdit(margin)}>
                                    <td className="px-3 py-2 font-medium">{margin.categoryName || "(신규)"}</td>
                ...
                <div className="flex-1 overflow-auto p-4">
                    {selectedId && isEditing ? (
                        <div className="space-y-4">
                            <h3 className="font-medium">품목분류별 마진 설정</h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="mb-1 block text-sm font-medium">품목분류코드</label><input type="text" value={editForm.categoryCode || ""} onChange={(e) => setEditForm({ ...editForm, categoryCode: e.target.value })} className="w-full rounded border px-3 py-2 text-sm" /></div>
                                <div><label className="mb-1 block text-sm font-medium">품목분류명 *</label><input type="text" value={editForm.categoryName || ""} onChange={(e) => setEditForm({ ...editForm, categoryName: e.target.value })} className="w-full rounded border px-3 py-2 text-sm" /></div>
