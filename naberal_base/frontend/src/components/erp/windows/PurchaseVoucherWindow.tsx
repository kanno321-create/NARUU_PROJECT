...
import { api, ERPPurchaseCreate } from "@/lib/api";
...
export function PurchaseVoucherWindow() {
  ...
  useEffect(() => {
    ...
    api.erp.customers.list({ search: supplierSearch, limit: 50 })
      .then(res => {
        setApiSuppliers(res.items.map(c => ({
          id: c.id,
          code: c.code,
          name: c.name,
          phone: c.phone || '',
  ...
  }, [showSupplierModal, supplierSearch]);
  ...
  // 결제 모달 폼 상태
  const [cashForm, setCashForm] = useState({ type: "현금출금", amount: 0 });
  const [bankForm, setBankForm] = useState({ type: "은행으로 지급", fromAccount: "", toAccount: "", amount: 0, fee: 0 });
  const [billForm, setBillForm] = useState({ type: "어음발행", billNo: "", amount: 0, dueDate: new Date().toISOString().split("T")[0] });
  const [cardForm, setCardForm] = useState({ type: "카드출금", cardType: "", amount: 0, approvalNo: "", memberName: "", paymentDate: new Date().toISOString().split("T")[0], installment: 1, feeRate: 0 });
  ...
  const purchaseAmount = productAmount + expenses + cardFee + bankFee;
  ...
  const unpaidAmount = purchaseAmount - totalPayment;
  ...
  const handleSelectProduct = (product: Product) => {
    const newItem: ProductItem = {
      ...
      memo: "",
      vat: Math.round(product.price * vatRate / 100),
    };
  ...
  const handleSaveCash = () => {
    ...
    setCashForm({ type: "현금출금", amount: 0 });
  ...
  const handleSaveBank = () => {
    ...
    setBankForm({ type: "은행으로 지급", fromAccount: "", toAccount: "", amount: 0, fee: 0 });
  ...
  const handleSaveBill = () => {
    ...
    setBillForm({ type: "어음발행", billNo: "", amount: 0, dueDate: new Date().toISOString().split("T")[0] });
  ...
  const handleSaveCard = () => {
    ...
    setCardForm({ type: "카드출금", cardType: "", amount: 0, approvalNo: "", memberName: "", paymentDate: new Date().toISOString().split("T")[0], installment: 1, feeRate: 0 });
    setShowCardModal(false);
  };

  // 저장
  const handleSave = async () => {
    if (!selectedSupplier) { alert("거래처를 선택하세요."); return; }
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div className="flex items-center justify-between border-b bg-gray-50 px-4 py-2">
        <h2 className="text-lg font-semibold text-gray-800">매입전표</h2>
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        <div className="mb-4 rounded-lg border bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-4 gap-4">
            <div>
              <label className="mb-1 block text-xs text-gray-500">매입일자</label>
              <input
                type="date"
                value={purchaseDate}
            ...
            <div>
              <label className="mb-1 block text-xs text-gray-500">매입대상거래처(O)</label>
            ...
            <div>
              <label className="mb-1 block text-xs text-gray-500">매입담당사원(U)</label>
              <div className="flex gap-1">
                <input
                  type="text"
          ...
          <div className="rounded-lg border bg-white p-4 shadow-sm">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{formatNumber(supplyTotal)}원</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">세액:</span>
                <span>{formatNumber(vatTotal)}원</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-orange-500">할인액(-):</span>
                <input
                  type="number"
                  value={discount}
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-lg text-red-600">{formatNumber(purchaseAmount)}원</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex rounded-md border">
                {["cash", "bank", "bill", "card"].map((tab) => (
                  <button
                    key={tab}
                    onClick={() => {
                      setPaymentTab(tab as "cash" | "bank" | "bill" | "card");
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div className="rounded-lg border bg-gray-50 p-4">
            <h3 className="mb-2 text-sm font-semibold text-gray-700">선택된 업체</h3>
            {selectedSupplier ? (
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between"><span className="text-gray-500">미수금액:</span><span className="font-medium">{formatNumber(selectedSupplier.receivable)}원</span></div>
                <div className="flex justify-between"><span className="text-gray-500">팩스번호:</span><span>{selectedSupplier.fax}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">미지급액:</span><span className="text-red-500 font-medium">{formatNumber(selectedSupplier.payable)}원</span></div>
                <div className="flex justify-between"><span className="text-gray-500">전화번호:</span><span>{selectedSupplier.phone}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">신용한도:</span><span>{formatNumber(selectedSupplier.creditLimit)}원</span></div>
                <div className="flex justify-between"><span className="text-gray-500">최종거래:</span><span>{selectedSupplier.lastTransaction}</span></div>
              </div>
            ) : (
          ...
          <div className="rounded-lg border bg-gray-50 p-4">
            <h3 className="mb-2 text-sm font-semibold text-gray-700">선택된 상품</h3>
            {selectedProduct ? (
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div className="flex justify-between"><span className="text-gray-500">최종매입:</span><span>{selectedProduct.lastPurchase}</span></div>
      ...
      >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {apiSuppliers.map((supplier) => (
                  ...
                  >
                    <td className="px-3 py-2 font-mono text-xs">{supplier.code}</td>
                    <td className="px-3 py-2">{supplier.name}</td>
                    <td className="px-3 py-2">{supplier.phone}</td>
                    <td className="px-3 py-2">{supplier.representative}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="flex justify-end border-t px-4 py-3">
          <span className="mr-auto text-sm text-gray-500">총 {apiSuppliers.length}개</span>
          <button onClick={() => setShowSupplierModal(false)} className="rounded-md bg-gray-200 px-4 py-2 text-sm hover:bg-gray-300">닫기</button>
        </div>
      </DraggableModal>
      ...
      {/* 전자세금계산서 모달 - 매입전표: 공급자=거래처(매입처), 공급받는자=우리회사 */}
      <TaxInvoiceModal
        isOpen={showTaxInvoice}
        onClose={() => setShowTaxInvoice(false)}
        transactionDate={purchaseDate}
        documentNumber={`P-${Date.now().toString().slice(-8)}`}
        supplierBusinessNumber={""}
        supplierCompanyName={selectedSupplier?.name || ""}
        supplierCeoName={selectedSupplier?.representative || ""}
        supplierAddress={""}
        supplierBusinessType={""}
        supplierBusinessItem={""}
        customerBusinessNumber={companyInfo.businessNumber || ""}
        customerCompanyName={companyInfo.companyName || ""}
        customerCeoName={companyInfo.ceoName || ""}
        customerAddress={`${companyInfo.address || ""} ${companyInfo.addressDetail || ""}`.trim()}
        customerBusinessType={companyInfo.businessType || ""}
        customerBusinessItem={companyInfo.businessCategory || ""}
        items={items.map((item) => ({
          id: item.id,
