...
const ORIGINAL_ITEMS: BankBalanceItem[] = [
  {
    id: "1",
    accountCode: "ba001",
    bankName: "부산은행",
    accountNo: "1235845692",
...
export function BankBalanceCarryover() {
  const [items, setItems] = useState<BankBalanceItem[]>(ORIGINAL_ITEMS);
  ...
  const handleCellChange = (id: string, field: keyof BankBalanceItem, value: string | number) => {
    setItems(
      items.map((item) =>
  ...
  return (
    <div className="flex h-full flex-col bg-gray-100">
      ...
      <div className="flex items-center justify-between border-b bg-gradient-to-r from-blue-600 to-blue-400 px-3 py-1">
        <span className="text-sm font-medium text-white">은행잔고이월</span>
      </div>

      {/* 검색 바 */}
      <div className="flex items-center justify-between border-b bg-white px-2 py-1">
        <div className="flex items-center gap-2">
          <span className="text-xs">은행명:</span>
          <input
            type="text"
            value={searchQuery}
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full border-collapse text-xs">
          <thead className="sticky top-0 bg-[#E8E4D9]">
            <tr>
              <th className="border border-gray-400 px-2 py-1 w-24">계좌코드</th>
              <th className="border border-gray-400 px-2 py-1 w-28">은행명</th>
              <th className="border border-gray-400 px-2 py-1">계좌번호</th>
              <th className="border border-gray-400 px-2 py-1 w-28">이월일자</th>
              <th className="border border-gray-400 px-2 py-1 w-28">이월계좌잔고</th>
            </tr>
          </thead>
          <tbody>
            {filteredItems.map((item) => (
              ...
              >
                <td className="border border-gray-300 px-2 py-1">{item.accountCode}</td>
                ...
                <td className="border border-gray-300 px-2 py-1 text-right text-blue-600">
                  {renderEditableCell(item, "carryoverBalance", "number")}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="border-t bg-gray-100 px-3 py-1 text-xs text-green-600">
        총 {filteredItems.length}개 계좌 | 더블클릭하여 잔고 수정 가능
      </div>
    </div>
  );
}
